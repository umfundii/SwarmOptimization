...
#  mutual information as the metric.  Adds scaling as an
#  additional dimension of the search space.
#
#  RTK, 09-Sep-2020
...
class Objective:
    ...
    def Evaluate(self, p):
        ...
        a,b = im.size
        img = np.array(im.resize((int(a*scale),int(b*scale)), resample=Image.BICUBIC))
...
def main():
    ...
    if (len(sys.argv) == 1):
        print()
        print("rigid_scale <src> <output> <npart> <niter> <alg> RI|SI|QI")
        print()
    ...
    ndim = 4 # (angle, x shift, y shift, scale)
    ...
    for k in range(len(simgs)):
        ...
        angle, xshift, yshift, scale = res["gpos"][-1]
        t = "  %3d: (NMI=%0.6f)  theta=%10.5f, x=%10.5f, y=%10.5f, scale=%10.5f\n" % (k, -res["gbest"][-1], angle, xshift, yshift, scale)
