#
#  file:  fxy_gaussian_ga_f.py
#
#  Gaussian function example for GA and F.
#
#  RTK, 19-Jun-2020
...
def main():
    ...
    for k,f in enumerate(fs):
        ...
        s = w = ff = 0
        for i in range(runs):
            swarm = GA(obj=obj, npart=npart, ndim=2, max_iter=miter, init=ri, bounds=b, F=f)
            swarm.Optimize()
            res = swarm.Results()
            x,y = res["gpos"][-1]
            if (wdist(x,y) < 1):
    ...
    for k,f in enumerate(fs):
        ...
        s = w = ff = 0
        for i in range(runs):
            swarm = GA(obj=obj, npart=npart, ndim=2, max_iter=miter, init=ri, bounds=b, F=f, CR=0.0)
            swarm.Optimize()
            res = swarm.Results()
            x,y = res["gpos"][-1]
            if (wdist(x,y) < 1):
    ...
    plt.savefig("fxy_gaussian_ga_f_plot.png", dpi=300)
    plt.close()
    print()
    ...
    plt.tight_layout(pad=0, w_pad=0, h_pad=0)
    plt.savefig("fxy_gaussian_ga_f_dispersion_plot.png", dpi=300)
    plt.close()
    ...
    plt.tight_layout(pad=0, w_pad=0, h_pad=0)
    plt.savefig("fxy_gaussian_ga_f_best_plot.png", dpi=300)
    plt.close()
