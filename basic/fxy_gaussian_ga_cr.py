#
#  file:  fxy_gaussian_ga_cr.py
#
#  Gaussian function example for GA and CR.
#
#  RTK, 19-Jun-2020
...
def main():
    ...
    for k,cr in enumerate(crs):
        ...
        s = w = f = 0
        for i in range(runs):
            swarm = GA(obj=obj, npart=npart, ndim=2, max_iter=miter, init=ri, bounds=b, CR=cr)
            swarm.Optimize()
            res = swarm.Results()
            x,y = res["gpos"][-1]
            if (wdist(x,y) < 1):
    ...
    plt.savefig("fxy_gaussian_ga_cr_plot.png", dpi=300)
    plt.close()
    print()
    ...
    plt.tight_layout(pad=0, w_pad=0, h_pad=0)
    plt.savefig("fxy_gaussian_ga_cr_dispersion_plot.png", dpi=300)
    plt.close()
    ...
    plt.tight_layout(pad=0, w_pad=0, h_pad=0)
    plt.savefig("fxy_gaussian_ga_cr_best_plot.png", dpi=300)
    plt.close()
