...
def main():
    ...
    if (len(sys.argv) == 1):
        print()
        print("store <products> <nshoppers> <npart> <niter> <alg> RI|SI|QI")
        print()
    ...
    #  Fix the initial swarm configuration
    ...
    #  But yes the evolution of the swarm
    np.random.seed(73939133)

    if (alg == "PSO"):
        swarm = PSO(obj=obj, npart=npart, ndim=ndim, init=i, max_iter=niter, bounds=b, inertia=LinearInertia())
    elif (alg == "RO"):
        swarm = RO(obj=obj, npart=npart, ndim=ndim, init=i, max_iter=niter, bounds=b)
    elif (alg == "GWO"):
        swarm = GWO(obj=obj, npart=npart, ndim=ndim, init=i, max_iter=niter, bounds=b)
    elif (alg == "JAYA"):
        swarm = Jaya(obj=obj, npart=npart, ndim=ndim, init=i, max_iter=niter, bounds=b)
    elif (alg == "GA"):
        swarm = GA(obj=obj, npart=npart, ndim=ndim, init=i, max_iter=niter, bounds=b)
    elif (alg == "DE"):
        swarm = DE(obj=obj, npart=npart, ndim=ndim, init=i, max_iter=niter, bounds=b)
    else:
